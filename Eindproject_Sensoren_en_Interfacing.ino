...
// This function allows you to switch between types of controls in your Blynk App
BLYNK_WRITE(V0) {
  bediening = !bediening;
}
...
void setup() {
  Serial.begin(9600);
  Blynk.begin(auth, ssid, pass, "...", 8081);// Make sure your port is also set to 8081 and that you enter your own domain on the ...
  dht.begin();
...
void sensorLezing() {
  ...
  gemetenLicht = map(analogRead(ldrPin), 0, 4096, 100, 0);
  gemetenBodemv = map(analogRead(bvPin), 0, 4096, 100, 0);
